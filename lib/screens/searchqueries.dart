...
class SearchQueries extends StatefulWidget {
  final String Text; // Assuming you have a City class

  SearchQueries({required this.Text});

  @override
  _SearchQueriesState createState() => _SearchQueriesState();
}

class _SearchQueriesState extends State<SearchQueries> {
        ...
        children: [
          Expanded(
            child: TextField(
              ...
              onChanged: (query) {
                filterList(removeLastSpace(query));
              },
          ...
          ElevatedButton(
            onPressed: () {
              filterList(removeLastSpace(_controller.text));
            },
  ...
  void filterList(String query) {
    setState(() {
      placeses_filtred = placeses.where((place) => place.name.toLowerCase().contains(query.toLowerCase()) || place.location.toLowerCase().contains(query.toLowerCase())).toList();
    });
  }
